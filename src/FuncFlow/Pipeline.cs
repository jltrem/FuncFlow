...
public class ExecutionPipeline<TContext> : IExecutionPipeline<TContext>
{
    ...
    public async Task<TContext> ExecuteAsync(TContext context)
    {
        for (int i = 0; i < _steps.Count; i++)
        {
            ...
            {
                Console.WriteLine($"Ending early with PipelineException: {e.Message}");
                return context;
            }
        }
        return context;
        ...
        TContext context, int stepIndex)
    {
        ...
        catch (TargetInvocationException tie) when (tie.InnerException != null)
        {
            ...
            if (tie.InnerException is PipelineException)
            {
                throw tie.InnerException;
            }
            throw new PipelineStepException($"INNER EXCEPTION: Error executing pipeline step {stepIndex}", tie.InnerException);
        }
        catch (Exception ex) 
        {
            ...
            throw new PipelineStepException($"OUTER EXCEPTION: Error executing pipeline step {stepIndex}", ex);
        }
    }
}
...
{
    public PipelineStepException(string message, Exception? innerException = null) : base(message, innerException) { }
}
